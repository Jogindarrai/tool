...
class Pages extends dbc
{
	 ...
	 {
		    @extract($data);
	        $query=parent::db_query("select * from #_".tblName." where email='".$email."' and phone='".$phone."'"); 
	        if($query->rowCount()==0)
	        {
				...
				$insertid=$GLOBALS['dbcon']->lastInsertId();
				 if($insertid<10){
					 $upid2='000'.$insertid;
					 } else if($insertid<100){
						$upid2='00'.$insertid; 
						 }
						 else if($insertid<1000){
						$upid2='0'.$insertid; 
						 }
						  else {
							$upid2=$insertid; 
	  ...
	  public function assign($updateid,$salesecutiveid,$colname)
	 {
		   ...
		   if(is_array($updateid))
		   {
			   $updateid=str_replace("RWI","",implode(',',$updateid));
		   }
	  ...
	  public function  display($start,$pagesize,$fld,$otype,$search_data,$extra,$extra1,$extra2)
	 {
		...
		if(trim($search_data)!='')
		{
		   $wh=" and (name like '%".$search_data."%' or email like '%".$search_data."%' or rwi like '%".$search_data."%' or phone like '%".$search_data."%'or occupation like '%".$search_data."%') ";	
		}
