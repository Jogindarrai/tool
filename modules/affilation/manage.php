...
if($RW->is_post_back())
{
	...
	if(isset($_POST['escalate'])){
		$query =$PDO->db_query("select * from #_affilation where pid ='".$_POST['lid']."' "); 
	...
	$lead['esclate']=1;
	$PDO->sqlquery("rs",'affilation',$lead,'pid',$_POST['lid']);
...
if(isset($_POST['qmsg'])){
	//print_r($_POST);
		$query =$PDO->db_query("select * from #_affilation where pid ='".str_replace('RWI','',$_POST['leadid'])."' "); 
	$row = $PDO->db_fetch_array($query);
	@extract($row);
	$ps['comment']=$_POST['comment'];
	$ps['lid']=str_replace('AF','',$_POST['leadid']);
	$ps['sentBy']=$_SESSION["AMD"][0];
	$ps['status']=9;
	$ps['type']=34;
	$PDO->sqlquery("rs",'activity',$ps);
...
$RW->redir($ADMIN->iurl($comp.(($start)?'&start='.$start:'').(($subpage_id)?'&subpage_id='.$subpage_id:'')), true); 
	...
	$_POST['leadid']=str_replace('AF','',$_POST['leadid']);
	//print_r($_POST); exit;
	$ps['lid']=$_POST['leadid'];
	$ps['sentBy']=$_SESSION["AMD"][0];
	$ps['status']=4;
	...
	$lead['status']=4;
	$PDO->sqlquery("rs",'affilation',$lead,'pid',$_POST['leadid']);
...
';
		...
		$mails=mail($to, $subject, $mailbody, $headers, '-fsupport@registrationwala.com');
		$st['status']=5;
$PDO->sqlquery("rs",'affilation',$st,'pid',str_replace('AF','',$_POST['leadid']));	
...
if($occupation){
 $extra = " and occupation='".$occupation."'"; 
	}	
        ...
        <div class="card">
          ...
          <div class="ManageLeads-wrp ">
            ...
            <div class="row mbt15 leadfilter">
              ...
              <div class="col">
                <label>City</label>
                <select name="city" onchange="getfilter(this.value,'city');" class="form-control" >
                 <option value="">All</option>
                   <?php 
 $sem_query = $PDO->db_query("select * from #_cities where status=1");
                while($sem_rows = $PDO->db_fetch_array($sem_query)){ ?>
                                <option value="<?=$sem_rows['pid']?>"  <?=($sem_rows['pid']==$city)?'selected="selected"':''?>><?=$sem_rows['name']?></option>
                                <?php } ?>
                </select>
              </div>
              ...
              <div class="col">
                ...
                <select name="occupation" onchange="getfilter(this.value,'occupation');" class="form-control">
                  <option value="">All</option>
                  <?php 
 $sem_query = $PDO->db_query("select * from #_affilation where occupation!='' GROUP BY occupation");
                while($sem_rows = $PDO->db_fetch_array($sem_query)){ ?>
                                <option value="<?=$sem_rows['occupation']?>"  <?=($sem_rows['occupation']==$occupation)?'selected="selected"':''?>><?=$sem_rows['occupation']?></option>
                                <?php } ?>
                </select>
              </div>
			...
			?>
                    <tr <?=$style?>>
                      ...
                      <td><?=$phone?></td>
                      <td><?=$companyName?></td>
                      <td><?=$PDO->getSingleresult("select name from rw_cities where pid='".$city."'")?> </td>
                      <td><?=$occupation?> </td>
                      <td>
