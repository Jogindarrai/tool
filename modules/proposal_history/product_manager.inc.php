...
class Pages extends dbc
{
	 ...
	 {
		    @extract($data);
	        $query=parent::db_query("select * from #_proposalsent where pid ='".$pid."' "); 
	        if($query->rowCount()==0)
	        {
			
				$data['created_on']=date('Y-m-d H:i:s');
				$data['create_by']=$_SESSION["AMD"][0];
				$data['shortorder']=parent::getSingleresult("select max(shortorder) as shortorder from #_proposalsent where 1=1 ")+1;
				parent::sqlquery("rs","proposalsent",$data);
		        parent::sessset('Proposal has been sent', 's');
	            $flag =1;
		    }else {
	 ...
	 {
		   @extract($data);
	       $query=parent::db_query("select * from #_proposalsent where  pid ='".$pid."' and pid='".$updateid."' "); 
	       if($query->rowCount()==0)
	       {
				 //$data['password']=$this->password($data['password']);
		         // parent::sqlquery("rs",'admin_users',$data);
				 $data['modified_on']=date('Y-m-d H:i:s');
			     parent::sqlquery("rs","proposalsent",$data,'pid',$updateid);
	 ...
	 public function  delete($updateid)
	 {
		   if(is_array($updateid))
		   {
		    ...
		    parent::db_query("delete from #_proposalsent where pid in ($updateid)");
	 ...
	 public function status($updateid,$status)
	 {
		   ...
		   parent::db_query("update  #_proposalsent set status='".$status."' where pid in ($updateid)");
	 ...
	 public function  display($start,$pagesize,$fld,$otype,$search_data,$zone,$mtype,$extra,$extra1,$extra2)
	 {
		...
		$sql = " from #_proposalsent where 1 ".$zone.$mtype.$extra.$extra1.$extra2.$wh;
