...
include(FS_ADMIN._MODS."/work_process/work_process.inc.php");
              ...
              <div class="row">
                ...
                <div class="col-12 col-md-6">
                  <div class="form-group">
                   ...
                   <select name="deptId"  class="validate[required] form-control" data-errormessage-value-missing="Department is required!">
            <option  value="">-------Select Department------</option>
            <?php 
 $sm_query = $PDO->db_query("select * from #_department where status='1'");
                while($sm_rows = $PDO->db_fetch_array($sm_query)){ ?>
             <option value="<?=$sm_rows['pid']?>" <?=($sm_rows['pid']==$deptId)?'selected="selected"':''?>><?=$sm_rows['name']?></option>
                                <?php } ?>
          </select>
                  </div>
                </div>
