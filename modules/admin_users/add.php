...
if ($RW->is_post_back()) {
    ...
    if (!empty($_FILES['userdoc']['name'])) {
        ...
        if ($uid > 0) {
            $delete_image = $PDO->getSingleResult(
                "SELECT userdoc FROM pms_admin_users WHERE user_id = $uid"
            );
    ...
    if ($uid > 0) {
        ...
        if ($_POST['password'] == '') {
            $_POST['password'] = $PDO->getSingleResult(
                "SELECT password FROM pms_admin_users WHERE user_id = $uid"
            );
        } else {
    ...
    if ($flag == 1) {
        $RW->redir($ADMIN->iurl($comp) . $dlr, true);
    }
}
...
if ($uid > 0) {
    $query = $PDO->db_query(
        "SELECT * FROM pms_admin_users WHERE user_id = $uid"
    );
