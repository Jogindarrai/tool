          ...
          <div class="user-wrp saleextutive mt-3 ">
                ...
                <table id="example" class="table table-striped table-bordered" >
                  <thead>
                    <tr>
                      ...
                      <th><a href="<?=$ADMIN->iurl($comp)?>&fld=score<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='score')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>> Score</span></a> </th>
               <th><a href="<?=$ADMIN->iurl($comp)?>&fld=agroup<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='agroup')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>> Activity Group</span></a> </th>
             <th><a href="<?=$ADMIN->iurl($comp)?>&fld=leadStage<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='leadStage')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>>Lead Stage</span></a> </th>
			...
			?>
                    <tr>
                      ...
                      <td><?=$name?></td>
                      <td><?=$score?> </td> 
                      <td><?=$PDO->getSingleresult("select name from #_activites_group where pid='".$agroup."'")?> </td>
                      <td><?=$PDO->getSingleresult("select name from #_lead_stage where pid='".$leadStage."'")?> </td>
