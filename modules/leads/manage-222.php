...
if($RW->is_post_back())
{
	...
	if(isset($_POST['escalate'])){
		$query =$PDO->db_query("select * from #_leads where pid ='".$_POST['lid']."' "); 
	...
	$lead['esclate']=1;
	$PDO->sqlquery("rs",'leads',$lead,'pid',$_POST['lid']);
...
if(isset($_POST['qmsg'])){
	//print_r($_POST);
		$query =$PDO->db_query("select * from #_leads where pid ='".str_replace('RWI','',$_POST['leadid'])."' "); 
	$row = $PDO->db_fetch_array($query);
	@extract($row);
	$ps['comment']=$_POST['comment'];
	$ps['lid']=str_replace('RWI','',$_POST['leadid']);
	$ps['sentBy']=$_SESSION["AMD"][0];
	$ps['status']=9;
	$ps['type']=34;
	$PDO->sqlquery("rs",'activity',$ps);
...
$RW->redir($ADMIN->iurl($comp.(($start)?'&start='.$start:'').(($subpage_id)?'&subpage_id='.$subpage_id:'')), true); 
	...
	$_POST['leadid']=str_replace('RWI','',$_POST['leadid']);
	//print_r($_POST); exit;
	$ps['lid']=$_POST['leadid'];
	$ps['sentBy']=$_SESSION["AMD"][0];
	$ps['status']=4;
...
';
		...
		$mails=mail($to, $subject, $mailbody, $headers, '-fsupport@registrationwala.com');
		$st['status']=5;
$PDO->sqlquery("rs",'leads',$st,'pid',str_replace('RWI','',$_POST['leadid']));	
...
if($department!=2 && in_array($_SESSION["AMD"][3],array("1", "6")) && $department!=''){
 ...
 $extra = " and service IN($serviceids)"; 
	}	
$start = intval($start);
...
<div class="col-3">
        ...
        <?php
		$srsqry=$PDO->db_query("select l.source, ls.name  from #_leads l JOIN #_lead_source ls ON l.source=ls.pid where 1 GROUP BY l.source ASC");
				while($srdata=$PDO->db_fetch_array($srsqry)){?>
					
				<option value="<?=$srdata['source']?>"><?=$srdata['name']?></option>	
				<?php	}
		?>
        </select>
        </div>
...
<?php 
    ...
    <div class="row">
      ...
      <div class="col-sm-6 col-md-6 col-xs-12 float-right">
             <label>Select Department</label>
                <select name="department" onchange="getfilter(this.value,'department');" class="form-control">
                  <option>-------</option>
                   <?php 
 ...
 <!-- <div class="col-lg-2 float-right">
  ...
  <a href="<?=SITE_PATH_ADM._MODS?>/leads/leads.csv" class="btn btn-info">Download Sample</a>
 ...
 $sem_query = $PDO->db_query("select * from pms_admin_users where user_type='".$user_type."' and rmanager='".$PDO->getSingleresult("select user_id from pms_admin_users where user_type='".$suser_type."' and dpid='".$department."'")."' order by name ASC");
              ...
              <div class="col">
                ...
                <select name="leadSource" onchange="getfilter(this.value,'leadSource');" class="form-control">
                  <option value="">All</option>
                  <?php 
 $sem_query = $PDO->db_query("select * from #_lead_source where status='1' and pid!=9 order by name ASC");
                while($sem_rows = $PDO->db_fetch_array($sem_query)){ ?>
			...
			?>
                    <tr <?=$style?>>
                      ...
                      <td><?=$email?></td>
                      <td><?=$phone?></td>
                      <td><?=$services==''?$service:$services?></td>
...
<?php if($department){
	$dpt='&department='.$department;
 	...
 	window.location.href = '<?=SITE_PATH_ADM."index.php?comp=".$comp?>&'+b+'='+a+'<?=$dpt?>';	
				}
</script>
