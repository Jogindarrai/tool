...
$sql = "SELECT * FROM #_leads where pid ='".$_POST['projectid']."' ";
...
else if($_POST['step']=='taxbasicDoc'){
$contt='<p>Thank you for associating with Registration Wala. This mail is to inform you that we have received your documents for the GST filing. After checking these documents properly, we shall begin with the filing procedure shortly. </p>';
$signature=$RW->emailSignature($ADMIN->user_type($_SESSION["AMD"][3]));
$content= mailcontent($list['name'],$contt,$signature);
$subject='Your basic Documents are Received | Registrationwala';
}
else if($_POST['step']=='taxPendingDoc'){ 

$contt='<p>This email is regarding recent request and documents you submitted for GST filing. Upon checking the documents, we found that the following documents will also be required from your end. </p>';
$signature=$RW->emailSignature($ADMIN->user_type($_SESSION["AMD"][3]));
$content= mailcontent($list['name'],$contt,$signature);
$subject='Your Documents are Pending for Filling | Registrationwala';

}
else if($_POST['step']=='formSubmitted'){ 

$contt='<p>This email is to inform you that your application for GST return has been filed successfully. </p><p>PFA acknowledgement</p>';
$signature=$RW->emailSignature($ADMIN->user_type($_SESSION["AMD"][3]));
$content= mailcontent($list['name'],$contt,$signature);
$subject='Your GST filing Form Submitted | Registrationwala';
}
else if($_POST['step']=='workComplete'){ 

$contt='<p>This email is to inform you that your GST has been filed successfully.</p><p>Please find the attached document</p>';
$signature=$RW->emailSignature($ADMIN->user_type($_SESSION["AMD"][3]));
$content= mailcontent($list['name'],$contt,$signature);
$subject='Your GST filing has been completed | Registrationwala';
}
